    ...
    for (const decorator of node.decorators) {
      const call =
        decorator.expression.kind == ts.SyntaxKind.CallExpression
          ...
          : undefined;
      const name =
        (call && call.expression.getText()) || decorator.expression.getText();
      if (name === 'Prop') {
        if (
          !node.modifiers ||
          ...
          ctx.addFailureAtNode(
            property.name,
            'Vue property should be readonly'
          );
        if (
          call &&
          call.arguments.length > 0 &&
          call.arguments[0].properties
            .map(x => x.name.getText())
            .some(x => x === 'default' || x === 'required')
        ) {
          if (property.questionToken !== undefined)
            ctx.addFailureAtNode(
              property.name,
              'Vue property is required and should not be optional.'
            );
        } else if (property.questionToken === undefined)
          ctx.addFailureAtNode(
            property.name,
            'Vue property should be optional - it is not required and has no default value.'
          );
      }
    }
  }
