...
const emailPattern =
  /^((?:[a-z0-9])+(?:[a-z0-9\._-])*@(?:[a-z0-9_-])+(?:[a-z0-9\._-]+)+)$/i;
const faUsernamePattern = /^([a-z0-9_\-~.]+)$/i;
const faSitePattern =
  /^https?:\/\/(?:www\.)?furaffinity\.net\/user\/([a-z0-9_\-~,]+)\/?$/i;
...
const twitterSitePattern =
  /^https?:\/\/(?:www\.)?twitter\.com\/([a-z0-9_]+)\/?$/i;
...
const inkbunnyNormalize = normalizeSiteUsernamePair(
  inkbunnySitePattern,
  inkbunnyUsernamePattern
);
const twitterNormalize = normalizeSiteUsernamePair(
  twitterSitePattern,
  twitterUsernamePattern
);

function normalizeSiteUsernamePair(
  site: RegExp,
  username: RegExp
